...
struct QueueState
{
    ...
    double S;	/* Starting time of the queue , not checked for wraparound*/
    double F;	/* Ending time of the queue, not checked for wraparound */
};
...
class WF2Q : public Queue 
{
	...
	protected:
		...
		bool port_ecn_marking_;	/* Enable per-port ECN marking or not */
		int mean_pktsize_;	/* MTU in bytes */
		int port_low_thresh_;	/* The low per-port ECN marking threshold (pkts)*/
		int port_ high_thresh_;	/* The high per-port ECN marking threshold (pkts)*/
