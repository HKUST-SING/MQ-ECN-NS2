...
class FlowThread(threading.Thread):
	def __init__(self, host, size, id, tos):
		self.host=host
		self.size=size
		self.id=id
		self.tos=tos
	...
	def run(self):
		...
		line=os.popen('./client.o '+str(self.host)+' '+str(5001)+' '+str(self.size)+' '+str(self.tos)).read()
		result=line.split()
		if len(result)>=4:
			#Get FCT for this flow and save it into global variable fcts
			fct=int(result[len(result)-4])
			fcts[self.id]=fct
...
if len(sys.argv)!=4:
	...
	for i in range(len(flows)):
		...
		newthread=FlowThread(host,flows[i],i,tos_arr[i])
		newthread.start()
